...
use project;
create table users
(userid int auto_increment primary key, name varchar(20), phonenumber varchar(20));
...
(addrid int auto_increment primary key, userid int, name varchar(50), 
contactPhoneNumber varchar(20), province varchar(100), city varchar(100),
streetaddr varchar(100), postCode varchar(12),
foreign key (userid) references users (userid));
...
(cardNumber varchar(25) primary key, expiryDate date, bank varchar(20));
...
(cardNumber varchar(25), userid int, organisation varchar(20),
...
(cardNumber varchar(25), userid int,
foreign key (cardNumber) references bankcard(cardNumber),
foreign key (userid) references users(userid));
...
create table store
(sid int auto_increment primary key, name varchar(20), province varchar(20),
city varchar(20), streetaddr varchar(20), customerGrade int, startTime date);
...
(spid int auto_increment primary key, streetaddr varchar(40), city varchar(30), province varchar(20),
...
(pid int auto_increment primary key, sid int, brand varchar(50), name varchar(100), type varchar(50),
modelNumber varchar(50), color varchar(50), amount int, price int,
...
(itemid int auto_increment primary key, pid int, price int, creationTime date,
...
(creationTime date,  userid int, pid int, grade float, content varchar(500),
...
(userid int, pid int, addTime date, quantity int,
foreign key (userid) references buyer(userid),
foreign key (pid) references product (pid));
...
create table manage 
(userid int, sid int, setupTime date,
...
create table orders
(orderNumber int primary key, paymentState varchar(12),
creationTime date, totalAmount int);
...
(orderNumber int, itemid int, quantity int);
...
(addrid int, orderNumber int, timeDelivered date,
...
(orderNumber int, creditcardNumber varchar(25), payTime date,
...
select distinct servicepoint.streetaddr, servicepoint.starttime, servicepoint.endtime
from users as u inner join address as a
on u.userid = a.userid
cross join servicepoint  where a.userid = 5;
...
from store as s inner join product as p
on s.sid = p.sid
inner join save_to_shopping_cart as sc
on p.pid = sc.pid where s.sid = 8;


#4.	Query the name and address of orders delivered on 2017-02-17
select a.name, a.streetaddr, a.city, a.postcode, a.province
from address as a inner join deliver_to as dt
on a.addrid = dt.addrid where dt.timeDelivered = '2017-02-17';


#5.	Query the comments of product 12345678?
select distinct p.pid, com.content
from product as p inner join comments as com
on p.pid = com.pid;

#6.	Insert the user id of sellers whose name starts with A into buyer
select  u.userid
from users as u inner join seller as s
on s.userid = u.userid where u.name like 'A%';
insert into buyer values('61');


#7.	Update the payment state of orders to unpaid which created after year 2016 and with total amount greater than 50.?
set sql_safe_updates = 0;
update orders set paymentState = 'unpaid' where creationTime like '2017%' and totalAmount > '50';


#8.	Update the name and contact phone number of address where the provice is Quebec and city is montreal.?
set sql_safe_updates = 0;
update address set name='Roshan', contactPhoneNumber = '9493558890' where province = 'Quebec' and city = 'Montreal';


#9.	Create view of all products whose price above average price.?
select avg(price) from product;
create view High_price_products as
select * from product where price > '504.25';


#10.	Create view of all products sales in 2016.?
create view sales_2016 as
select * from orderitem where creationTime like '2016%'
